...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<Vector<DataPoint>> left = sequences.subList(0, half);
    Vector<Vector<DataPoint>> right = sequences.subList(half);

    Vector<DataPoint> combinedLeft = combine(left);
    Vector<DataPoint> combinedRight = combine(right);
    Vector<DataPoint> combined;

    int leftSize = combinedLeft.size();
    int rightSize = combinedRight.size();
    int i1 = 0;
    int i2 = 0;

    // keep track of index of first elements without changing vectors
    while (i1 < leftSize && i2 < rightSize) {
        if (combinedLeft[i1].weight < combinedRight[i2].weight) {
            combined += combinedLeft[i1];
            i1++;
        } else {
            combined += combinedRight[i2];
            i2++;
        }
    }


    // add the rest of the sorted half-vector
    while (i1 < leftSize) {
        combined += combinedLeft[i1];
        i1++;
    }
