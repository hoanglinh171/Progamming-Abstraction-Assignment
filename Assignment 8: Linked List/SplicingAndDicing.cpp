...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide* end = nullptr;

    int i = 0;
    while (i < str.size()) {
        Nucleotide* cell = new Nucleotide;
        cell->value = str[i];
        cell->prev = end;
        cell->next = nullptr;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    Nucleotide* iter = target;

    if (target == nullptr || dna == nullptr) {
        return dna;
    }

    while (iter != nullptr && end != nullptr) {
        if (end->value == iter->value) {
            iter = iter->next;
            end = end->next;
        }
        else {
...
STUDENT_TEST("deleteNucleotides with a sequence of 1 element") {
...
STUDENT_TEST("deleteNucleotides with a sequence of 3 elements") {
    Nucleotide* dna = new Nucleotide;
    dna->value = 'A';
    dna->prev = nullptr;
    ...
    dna->next->next->value = 'G';
    dna->next->next->prev = dna->next;
    dna->next->next->next = nullptr;
