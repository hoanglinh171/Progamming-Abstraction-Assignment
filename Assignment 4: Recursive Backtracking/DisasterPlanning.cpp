...
// Assume supplyLocations is always empty for the first call

bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
                               const Set <string>& uncoveredCities,
                               int numCities,
                               Set<string>& supplyLocations) {

    // Base case 1: number of cities is negative
    if (numCities < 0) {
        error("The number of cities cannot be negative!");

    // Base case 2: There are no cities in the network or all cities are covered
    } else if (roadNetwork.isEmpty() || uncoveredCities.isEmpty()) {
        return true;

    // Base case 3: Not every city is covered but the number of cities allowed for supply is 0.
    } else if (!uncoveredCities.isEmpty() && numCities == 0) {
        return false;

    // Recursive case: Backtracking
    } else {
        string firstCity = uncoveredCities.first();
        Set <string> consideredCities = {firstCity};
        consideredCities += roadNetwork[firstCity];

        for (string city: consideredCities) {
            Set <string> uncoveredCitiesUpdate = uncoveredCities;
            Set <string> coveredCities = {city};
            coveredCities += roadNetwork[city];
            uncoveredCitiesUpdate -= coveredCities;
            if (canBeMadeDisasterReadyRec(roadNetwork, uncoveredCitiesUpdate, numCities - 1, supplyLocations)) {
