...
bool canAllPatientsBeSeen(const Map<string, int>& doctors,
    ...
    if (patients.isEmpty()) {
        ...
        // each choice is a patient choose the doctor
        string firstPatient = patients.firstKey();

        for (string eachDoctor: doctors) {
            Map<string, int> doctorTimeUpdate = doctors;
            if (patients[firstPatient] <= doctors[eachDoctor]) {
                doctorTimeUpdate[eachDoctor] -= patients[firstPatient];
                Map<string, int> chosenPatient = {{firstPatient, patients[firstPatient]}};
                if (canAllPatientsBeSeen(doctorTimeUpdate, patients - chosenPatient, schedule)) {
                    schedule[eachDoctor] += firstPatient;
                    return true;
                }
            }
        }
...
STUDENT_TEST("Test a random case.") {
    Map<string, Set<string>> schedule;
    ...
    Map<string, Set<string>> expectedSchedule = {
        { "Dr. Thomas", { "Washkansky", "Molaison"}},
        {"Dr. Taussig", {"Sandoval"}},
        {"Dr. Sacks", {"Giese", "Martin"}},
        {"Dr. Ofri", {"Gage", "Lacks", "Writebol"}}
    };
