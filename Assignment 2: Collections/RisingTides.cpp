...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    // a grid same size with terrain, contain bool of flood
    Grid<bool> floodedGrid(terrain.numRows(), terrain.numCols());

    // the queue of cell altitude to consider for flood
    Queue<GridLocation> floodSource;

    for (int i = 0; i < sources.size(); i++) {
        int sourceRow = sources[i].row;
        int sourceCol = sources[i].col;
        // if the terrain at altitude same with source is lower than water level,
        // the bool grid turn true at that altitude and the altitude is queued to consider
        if (terrain[sourceRow][sourceCol] <= height) {
            floodedGrid[sourceRow][sourceCol] = true;
            floodSource.enqueue(sources[i]);
        }
    }

     while (!floodSource.isEmpty()) {
         // take the flooded altitude from the queue
         GridLocation dequeueCell = floodSource.dequeue();
         int row = dequeueCell.row;
         int col = dequeueCell.col;

         for (int i = -1; i <= 1; i++) {
             // check the left and right altitude
             if (row+i >=0 && terrain.inBounds(row+i, col)) {  // inside the terrain
                 if (terrain[row+i][col] <= height) {
                     if (floodedGrid[row+i][col] != true) {
                        // check terrain value, change floodedGrid value and enqueue the altitude
                        floodedGrid[row+i][col] = true;
                        GridLocation floodedCell = { row+i, col };
                        floodSource.enqueue(floodedCell);
                     }
                 }
             }
             // check the up and down altitude
             if (col+i >= 0 && terrain.inBounds(row, col+i)) {
                 if (terrain[row][col+i] <= height) {
                     if (floodedGrid[row][col+i] != true) {
                        // check terrain value, change floodedGrid value and enqueue the altitude
                        floodedGrid[row][col+i] = true;
                        GridLocation floodedCell = { row, col+i };
                        floodSource.enqueue(floodedCell);
                     }
                 }
             }
